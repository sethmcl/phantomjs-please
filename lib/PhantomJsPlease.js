...
PhantomJsPlease.prototype.initialize = function( config ){
  ...
  this.phantomProcess = this.createPhantomProcess( {
   ...
   port: this.port,
   phantomBinaryPath: ( config ) ? config.phantomBinaryPath : null
  });

  this.once( 'proxy-connected', function(){
    deferred.resolve();
    this.running = true;
...
PhantomJsPlease.prototype.createSocketServer = function( config ){
  var server = io.listen( config, { 'log level': 0 } );
...
PhantomJsPlease.prototype.killPhantomProcess = function(){
...
PhantomJsPlease.prototype.createBrowser = function(){
  ...
  // Navigate
  browser.on( 'navigate', (function( event ){
    this.sendCommand( 'navigate', event );
  }).bind(this) );

  // Kill
  browser.on( 'kill', (function( event ){
    this.sendCommand( 'kill', event );
    delete this.browsers[ browser.id ];
